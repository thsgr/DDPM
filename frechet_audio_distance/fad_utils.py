...
def save_split(gen_config, out):

      fp = gen_config.output_audio
      fs = gen_config.fs
      batch_size = gen_config.batch_size

      # Saving each sound in batch in different file to compute fad
      out = out.reshape(batch_size, gen_config.num_samples)
      out = out.to(dtype=torch.float32)
      fp = fp.rsplit('.')[0]
      
      # Making sure folder exists
      if not Path(f'{fp}/').exists():
              os.mkdir(f'{fp}/')
      
      for i in range(batch_size):
          torchaudio.save(f'{fp}/audio_{i}.wav', out[i].unsqueeze(0), fs, channels_first=True)
