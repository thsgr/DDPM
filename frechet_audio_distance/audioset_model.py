...
class AudioSetModel(object):
  ...
  def __init__(self, checkpoint, step_size=None, normalize=True):
    ...
    with tf.Graph().as_default():
      ...
      self._features_tensor = self._sess.graph.get_tensor_by_name(
          vggish_params.params.INPUT_TENSOR_NAME)
      self._embedding_tensor = self._sess.graph.get_tensor_by_name(
          vggish_params.params.OUTPUT_TENSOR_NAME)

    features_sample_rate = 1.0 / vggish_params.params.STFT_HOP_LENGTH_SECONDS
    self._example_window_length = int(
        round(vggish_params.params.EXAMPLE_WINDOW_SECONDS * features_sample_rate))
    self._example_hop_length = int(
        round(vggish_params.params.EXAMPLE_HOP_SECONDS * features_sample_rate))
  ...
  def extract_features(self, np_samples):
    ...
    if self._step_size is not None:
      samples_splits = []
      for i in range(0, samples - vggish_params.params.SAMPLE_RATE + 1,
                      self._step_size):
        samples_splits.append(np_samples[i:i + vggish_params.params.SAMPLE_RATE])
    else:
      samples_splits = np.split(np_samples, samples / vggish_params.params.SAMPLE_RATE)
    # Compute log mel spectrogram features.
    for samples_window in samples_splits:
      log_mel = mel_features.log_mel_spectrogram(
          samples_window,
          audio_sample_rate=vggish_params.params.SAMPLE_RATE,
          log_offset=vggish_params.params.LOG_OFFSET,
          window_length_secs=vggish_params.params.STFT_WINDOW_LENGTH_SECONDS,
          hop_length_secs=vggish_params.params.STFT_HOP_LENGTH_SECONDS,
          num_mel_bins=vggish_params.params.NUM_MEL_BINS,
          lower_edge_hertz=vggish_params.params.MEL_MIN_HZ,
          upper_edge_hertz=vggish_params.params.MEL_MAX_HZ)
